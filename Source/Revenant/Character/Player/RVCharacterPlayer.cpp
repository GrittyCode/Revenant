...
void ARVCharacterPlayer::BeginPlay()
{
	...
	StaminaComponent->OnStaminaDepleted.AddUObject(GuardComponent, &URVGuardComponent::OnStaminaDepletedHandler);
...
void ARVCharacterPlayer::StartDodge(UAnimMontage* InMontage)
{
	...
	if (!StaminaComponent->ConsumeStamina(PlayerData->GetPlayerStatRow()->DodgeStaminaCost)) { return; }
...
void ARVCharacterPlayer::InputDodge(const FInputActionValue& Value)
{
	...
	if (LockOnComponent->IsLockedOn())
	{
		...
		{
			SetActorRotation((-DodgeDir).ToOrientationRotator());
			Montage = Angle > 0.f ? WeaponData->GetLockOnDodgeMontage(ERVDodgeDirection::BackRight) : WeaponData->GetLockOnDodgeMontage(ERVDodgeDirection::BackLeft);
		}
		else if (Angle < -112.5f && Angle >= -157.5f)
		{
			SetActorRotation((-DodgeDir).ToOrientationRotator());
